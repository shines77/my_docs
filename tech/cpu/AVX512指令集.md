...
在使用 AVX-512 编程时，编译器通常会提供一些预定义的宏（macros），用于检测处理器支持的 AVX-512 子集。这些宏可以帮助开发者在代码中根据硬件支持条件编译特定的指令集优化代码。除了 `__AVX512VL__`，以下是一些常见的 AVX-512 相关宏：
...
AVX-512（Advanced Vector Extensions 512）是英特尔推出的一组指令集扩展，旨在提供更强大的并行计算能力。除了 `AVX-512VL`（Vector Length Extensions），AVX-512 还包含多个子集，每个子集针对不同的应用场景和功能进行了优化。以下是一些主要的 AVX-512 子集：
