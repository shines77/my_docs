...
在 x86-64 上，这个表的近似内存开销是每个桶16个字节，加上键值对填充，除了空桶的键值对大小。
...
这是基准测试中包括的最古老和可能最受欢迎的C哈希表。像 std::unordered_map 一样，它是基于节点的分离链表表。与其他表相比，它提供的功能是基础的，用户必须自己编写大部分脚手架。它也是这些基准测试中唯一的侵入性表。
