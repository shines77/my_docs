...
1. **将完整流解码到一个内存缓冲区中。**
    ...
    如果我们将完整的 LZMA 流解码到内存中的一个输出缓冲区，解码器可以将该输出缓冲区用作滑动窗口。因此，解码器不需要为滑动窗口分配额外的缓冲区。
...
解码具有 `lp = 4` 和 `lc = 8` 的流需要最大的内存状态使用量：

```cpp
RAM_lc8_lp4 = 4 KiB + 1.5 KiB * 4096 = 6148 KiB
```

如果解码器使用 `LZMA2` 的受限属性条件 `(lc + lp <= 4)`，则内存使用量将不大于：
...
LZMA 算法使用范围编码 (Range Encoding) [(1)](#referen1) 作为熵编码方法。
